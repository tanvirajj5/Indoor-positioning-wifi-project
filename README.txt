...
📈 Results Summary
k-NN: Performs best with a small k (1 or 3) and shows improved accuracy with more data and 50-100 PCA components.

SVM: A tuned polynomial kernel outperforms a linear one. Using 150 PCA components yields better results than 50.

Tree-Based Models: Gradient Boosting with 50 PCA components achieves over 97% accuracy. The pruned Decision Tree performs optimally with minimal pruning.
...
Install with: pip install numpy pandas matplotlib scikit-learn kagglehub.
...
Open and run ml_project.ipynb in a Jupyter environment. The notebook will automatically download the data and run the analyses.